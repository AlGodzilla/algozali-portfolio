...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Role labels - positioned on sides for all screens */}
      {heroConfig.roles[0] && (
        <div
          className={cn(
            'absolute left-4 sm:left-8 lg:left-16 top-[25%] sm:top-[30%] z-30 transition-all duration-[1200ms] ease-out-quart',
            isLoaded ? 'opacity-100' : 'opacity-0'
          )}
          style={{ transitionDelay: '800ms' }}
        >
          <span className="text-[10px] sm:text-xs font-geist-mono uppercase tracking-[0.2em] sm:tracking-[0.3em] text-white/70">
            {heroConfig.roles[0]}
          </span>
        </div>
      )}
      {heroConfig.roles[1] && (
        <div
          className={cn(
            'absolute right-4 sm:right-8 lg:right-16 top-[25%] sm:top-[30%] z-30 transition-all duration-[1200ms] ease-out-quart',
            isLoaded ? 'opacity-100' : 'opacity-0'
          )}
      ...
      <div className="relative z-30 flex flex-col items-center justify-end min-h-screen px-6 lg:px-12 pointer-events-none">
        ...
        >
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-black text-white tracking-[-0.03em] leading-[1]">
            {heroConfig.name}
          </h1>
        </div>
      </div>
